...
* GPIO-based I2C bus recovery using up to nine SCL clock pulses
* Manual STOP generation during I2C bus recovery
* I2C1 peripheral reinitialization after bus recovery
* Runtime I2C bus-recovery diagnostics
* Physical stuck-SDA fault testing on hardware
* Automatic sensor communication recovery after the I2C bus is released
...
* `drivers/i2c1` initializes I2C1, performs register transactions, detects stuck-bus conditions, and performs GPIO-based bus recovery.
...
* `mcu/gpio.hpp` configures GPIO modes and output state and provides GPIO input-state reading used by I2C bus recovery.
...
Before starting a transaction, the I2C1 driver waits for the STM32 `BUSY`
status flag to clear.

If the bus remains busy until the polling timeout expires, the driver
attempts to recover the physical I2C bus instead of immediately failing the
transaction.
...
→ release both lines
→ check SDA
→ generate up to 9 SCL clock pulses while SDA remains low
→ generate a manual STOP condition
→ verify that SDA and SCL are released high
→ restore PB8/PB9 to I2C1 alternate function
→ reset and reconfigure the I2C1 peripheral
→ check the BUSY state again
```

The clock pulses allow a slave that was interrupted during a byte transfer
to advance its internal I2C state machine and release SDA. Nine pulses cover
the eight data bits and ACK/NACK phase of an I2C byte transfer.

The manual STOP condition is generated by releasing SDA from low to high
while SCL is high.

If recovery succeeds and the STM32 `BUSY` flag clears, the original
transaction continues normally. If the bus remains unavailable, the
operation returns `bus_busy_timeout`.

The driver maintains a recovery counter that can be queried through:

```cpp
std::uint32_t recovery_count();
```

The application reports changes to this counter through USART2 for runtime
diagnostics.

The recovery path was verified on physical hardware by temporarily forcing
SDA (`PB9`) low. While SDA was held low, BMP280 and MPU-6050 transactions
failed and recovery attempts were recorded. After SDA was released, normal
communication with both sensors resumed automatically without resetting or
reflashing the MCU.

This test verifies recovery from a persistent physical bus fault and
automatic operation after the fault is removed. It does not emulate every
possible internal slave state that could leave SDA asserted during an
...
1. Add an I2C OLED display for standalone sensor output.
